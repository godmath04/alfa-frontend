...
export class AppointmentStateService {
  ...
  private readonly _specialty = signal<SpecialtyState>({
    catalog: [],
    selected: null,
    loading: false,
    error: null,
  });
  private readonly _doctor = signal<DoctorState>({
    list: [],
    selected: null,
    loading: false,
    error: null,
  });
  private readonly _availability = signal<AvailabilityState>({
    times: [],
    selectedDate: null,
    selectedTime: null,
    loading: false,
  ...
  private readonly _creation = signal<CreationState>({
    motivo: null,
    result: null,
    loading: false,
    error: null,
  });
  private readonly _quick = signal<QuickState>({
    proposal: null,
    countdown: 0,
    loading: false,
    error: null,
  ...
  setSpecialties(catalog: SpecialtyCatalog[]): void {
    this._specialty.update((s) => ({ ...s, catalog }));
  }
  setLoading(loading: boolean): void {
    this._specialty.update((s) => ({ ...s, loading }));
  }
  setError(error: string | null): void {
    this._specialty.update((s) => ({ ...s, error }));
  }
  selectSpecialty(selected: SpecialtyCatalog): void {
    this._specialty.update((s) => ({ ...s, selected }));
  }

  // ─── Doctor ────────────────────────────────────────

  setDoctors(list: SpecialtyDoctor[]): void {
    this._doctor.update((s) => ({ ...s, list }));
  }
  setDoctorsLoading(loading: boolean): void {
    this._doctor.update((s) => ({ ...s, loading }));
  }
  setDoctorsError(error: string | null): void {
    this._doctor.update((s) => ({ ...s, error }));
  }
  selectDoctor(selected: SpecialtyDoctor): void {
    this._doctor.update((s) => ({ ...s, selected }));
  }

  // ─── Availability ──────────────────────────────────

  selectDate(selectedDate: string): void {
    this._availability.update((s) => ({ ...s, selectedDate }));
  }
  selectTime(selectedTime: string | null): void {
    this._availability.update((s) => ({ ...s, selectedTime }));
  }
  setAvailability(times: string[]): void {
    this._availability.update((s) => ({ ...s, times }));
  }
  setAvailabilityLoading(loading: boolean): void {
    this._availability.update((s) => ({ ...s, loading }));
  }
  setAvailabilityError(error: string | null): void {
    this._availability.update((s) => ({ ...s, error }));
  }
  setSlotDurationMinutes(slotDurationMinutes: number): void {
    this._availability.update((s) => ({ ...s, slotDurationMinutes }));
  }

  // ─── Creation ──────────────────────────────────────

  setMotivo(motivo: string | null): void {
    this._creation.update((s) => ({ ...s, motivo }));
  }
  setAppointmentResult(result: AppointmentResponse | null): void {
    this._creation.update((s) => ({ ...s, result }));
  }
  setCreating(loading: boolean): void {
    this._creation.update((s) => ({ ...s, loading }));
  }
  setCreateError(error: string | null): void {
    this._creation.update((s) => ({ ...s, error }));
  }

  // ─── Quick appointment ─────────────────────────────

  setProposal(proposal: QuickProposalResponse | null): void {
    this._quick.update((s) => ({ ...s, proposal }));
  }
  setProposalCountdown(countdown: number): void {
    this._quick.update((s) => ({ ...s, countdown }));
  }
  setProposalLoading(loading: boolean): void {
    this._quick.update((s) => ({ ...s, loading }));
  }
  setProposalError(error: string | null): void {
    this._quick.update((s) => ({ ...s, error }));
  }
