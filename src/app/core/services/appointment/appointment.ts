...
export class AppointmentService {
  ...
  getDoctorsBySpecialty(specialtyId: number): Observable<SpecialtyDoctor[]> {
    return this._http
      .get<
        MedicoResponse[]
      >(`${this.baseUrl}/api/agendamiento/especialidades/${specialtyId}/medicos`)
      .pipe(
        map((response) =>
          response.map((doctor) => ({
            id: doctor.id,
  ...
  getAvailability(doctorId: number, date: string): Observable<DoctorAvailability> {
    const params = new HttpParams().set('medicoId', doctorId.toString()).set('fecha', date);

    return this._http
      .get<DisponibilidadResponse>(`${this.baseUrl}/api/agendamiento/disponibilidad`, { params })
      .pipe(
        map((response) => ({
  ...
  createAppointment(request: CreateAppointmentRequest): Observable<AppointmentResponse> {
    return this._http
      .post<CitaResponse>(`${this.baseUrl}/api/agendamiento/citas`, request)
  ...
  getQuickProposal(specialtyId: number, date: string): Observable<QuickProposalResponse> {
    ...
    return this._http
      .get<PropuestaResponse>(`${this.baseUrl}/api/agendamiento/citas/rapida/propuesta`, { params })
      .pipe(
        map((r) => ({
  ...
  confirmQuickAppointment(request: ConfirmQuickRequest): Observable<AppointmentResponse> {
    return this._http
      .post<CitaResponse>(`${this.baseUrl}/api/agendamiento/citas/rapida/confirmar`, request)
      .pipe(map((r) => this._mapCitaResponse(r)));
  }
  ...
  getMisCitas(filtros: MisCitasFiltros): Observable<PageResponse<MisCitaItem>> {
    ...
    return this._http.get<PageResponse<MisCitaItem>>(
      `${this.baseUrl}/api/agendamiento/citas/mis-citas`,
      { params },
    );
  }

  cancelarCita(citaId: number): Observable<CancelacionCitaResponse> {
    return this._http.put<CancelacionCitaResponse>(
      `${this.baseUrl}/api/agendamiento/citas/${citaId}/cancelar`,
      {},
    );
  }
