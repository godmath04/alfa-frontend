...
export class UserViewModel {
  ...
  loadAll(): void {
    ...
    this._service.getUsers().subscribe({
      ...
      error: ()    => { this._state.setError('admin.users.error'); this._state.setLoading(false); },
    });
  }
  ...
  changeRole(id: number, role: string): void {
    ...
    this._service.changeUserRole(id, role).subscribe({
      next:  () => { this._state.updateRole(id, role); this._state.setSaving(false); },
      error: () => { this._state.setSaving(false); },
    });
  }
}
