...
import { AttentionSchedule } from '../../models/admin.model';

interface ScheduleState {
  items:     AttentionSchedule[];
  loading:   boolean;
  error:     string | null;
  saving:    boolean;
...
export class ScheduleStateService {
  ...
  private readonly _state = signal<ScheduleState>({
    items: [], loading: false, error: null, saving: false, saveError: null,
  });
  ...
  setItems(items: AttentionSchedule[]):   void { this._state.update(s => ({ ...s, items })); }
  ...
  upsert(schedule: AttentionSchedule): void {
    this._state.update(s => ({
      ...s,
      items: s.items.some(i => i.id === schedule.id)
        ? s.items.map(i => i.id === schedule.id ? schedule : i)
        : [...s.items, schedule],
    }));
  }
