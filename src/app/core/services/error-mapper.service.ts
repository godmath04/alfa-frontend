...
export class ErrorMapperService {
  ...
  mapCreateError(err: ApiError): string {
    if (err.status === 409) return this._translate.get('paciente.appointments.errors.slot-conflict');
  ...
  mapAvailabilityError(err: ApiError): string {
    if (err.status === 400 && err.error.message) {
      ...
      if (detail.includes('encontrado')) return this._translate.get('paciente.appointments.errors.doctor-not-found');
      if (detail.includes('atiende'))    return this._translate.get('paciente.appointments.errors.doctor-not-available');
