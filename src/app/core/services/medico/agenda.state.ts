...
export class AgendaStateService {
  ...
  setAgenda(agenda: DailyAgenda[]): void {
    this._state.update((s) => ({ ...s, agenda }));
  }
  ...
  setWeekStart(weekStart: string): void {
    this._state.update((s) => ({ ...s, weekStart }));
  }
  setSelectedDate(selectedDate: string): void {
    this._state.update((s) => ({ ...s, selectedDate }));
  }
  setSearchQuery(searchQuery: string): void {
    this._state.update((s) => ({ ...s, searchQuery }));
  }
  setStatusFilter(statusFilter: StatusFilter): void {
    this._state.update((s) => ({ ...s, statusFilter }));
  }
  setExpandedId(expandedId: number | null): void {
    this._state.update((s) => ({ ...s, expandedId }));
  }
  setActiveView(activeView: ActiveView): void {
    this._state.update((s) => ({ ...s, activeView }));
  }
}
